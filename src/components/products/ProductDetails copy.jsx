...
function ProductDetails() {
  ...
  const AddToCard = async () => {
    ...
    try {
      ...
      cartResponse = await axios.get(
        `${import.meta.env.VITE_API}/store/user/${user_id}/cart`
      );
      ...
      if (cartResponse.data.length >= 1) {
        if (cartResponse.data[0].id) {
          ...
          config = {
            method: "put",
            url: `${import.meta.env.VITE_API}/store/cart/update/${
        ...
        config = {
          method: "post",
          url: `${import.meta.env.VITE_API}/store/cart/create`,
        };
      }
  ...
  const handleSubmitReview = (event) => {
    ...
    let config = {
      method: "post",
      maxBodyLength: Infinity,
      url: import.meta.env.VITE_API + "/api/review/create",
      headers: {
        "Content-Type": "application/json",
      },
      data: data,
  ...
  useEffect(() => {
    ...
    let config = {
      method: "get",
      maxBodyLength: Infinity,
      url: import.meta.env.VITE_API + `/store/review/${product_id}`,
      headers: {
        "Content-Type": "application/json",
      },
      data: data,
                  ...
                  <form className="boxProduct_deteils">
                    ...
                    <div className="txtContentproduct">
                      ...
                      <div className="Count_product">
                        ...
                        <Link className="echbtn btnAdd" onClick={AddToCard}>
                          Add To Cart
                        </Link>
                      </div>
                    </div>
                  ...
                  <ul className="reviews">
                    {displayedReviews.map((review) => (
                      <li key={review.id} className="review-item">
                        <h3 className="rating">{review.nickname || "null"}:</h3>
                        <p className="comment">{review.review || "null"}</p>
