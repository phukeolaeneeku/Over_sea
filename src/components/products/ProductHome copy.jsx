...
const ProductHome = () => {
  ...
  const [goods_category, set_goods_category] = useState(0);
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
        // console.log(JSON.stringify(response.data));
        set_all_category(response.data);
      })
      .catch((error) => {
        console.log(error);
      });
  ...
  useEffect(() => {
    let config = {
      method: "get",
      maxBodyLength: Infinity,
      url: import.meta.env.VITE_API + `/store/?category=${filter}`,
      headers: {
        "Content-Type": "application/json",
      },
    };
  ...
  return (
    <div>
      ...
      {all_category.map((i, index) => (
        <div className="box-category" key={index} onClick={""}>
          <button>
